...
pub async fn get_category_id(category_id: i32, pool: &State<Pool<Postgres>>) -> Result<Json<Category>, Status> {
    let category = CategorieService::fetch_id(category_id, &pool).await;
    match category {
...
#[put("/categories/<category_id>", format = "json", data = "<category_payload>")]
pub async fn update_category(
    category_id: i32,
    category_payload: Json<Category>,
    pool: &State<Pool<Postgres>>,
) -> Result<Status, Status> {
    let result = CategorieService::update_category(category_id, category_payload.0, &pool).await;
    match result {
        Ok(_) => Ok(Status::Ok),
        Err(err) => {
            // output to log or stdout
